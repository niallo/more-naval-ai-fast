...
set TARGET=ReleaseFast
if not "%MNAI_TARGET%"=="" set TARGET=%MNAI_TARGET%
set OUT=temp_files\%TARGET%
set BIN=%TARGET%

set PATH=%TOOLKIT%\bin;%PSDK%\bin;%PATH%

set DEFINES=/DWIN32 /D_WINDOWS /D_USRDLL /DCVGAMECOREDLL_EXPORTS /DNDEBUG /DLOG_AI /DFINAL_RELEASE /DMNAI_PROFILE_TRUE_COMBAT_CACHE /DMNAI_PROFILE_UPGRADE_LIST_CACHE /DMNAI_PROFILE_PATHVALID_MOVE_CACHE /DMNAI_PROFILE_BASE_BONUS_CACHE /DMNAI_PROFILE_ACTIVE_DANGER_CACHE_SKIP_CLEAN /DMNAI_OPT_CITY_YIELD_DELTA_CONTEXT /DMNAI_OPT_NOBONUS_VOTE_SOURCE_FIRST /DMNAI_OPT_ATTACK_ODDS_BEFORE_PATH /DMNAI_OPT_PATHCOST_SINGLE_UNIT_NEXT_SKIP /DMNAI_OPT_PATHVALID_STICKY_UPDATE_CACHE /DMNAI_OPT_ROUTE_TERRITORY_OWNED_PLOT_CACHE /DMNAI_OPT_TOWER_MANA_OWNED_PLOT_SERVICE /DMNAI_OPT_PILLAGE_VALUE_BEFORE_PATH /DMNAI_OPT_PATHVALID_MOVE_BEFORE_DANGER /DMNAI_OPT_WORKER_OWNED_PLOT_SERVICE /DMNAI_OPT_IMPROVE_BONUS_OWNED_PLOT_SERVICE /DMNAI_OPT_CONTIGUOUS_TURN_UPDATES %MNAI_EXTRA_DEFINES%
